...
**Addendum, same date:** the thumbnail panels above are small; a second, additive script (`layer3_regime_wave_gallery_detailed.py`) re-plots the same 10 regimes at full size, matching the exact style of the original 3-regime characterization panels (raw gap sequence + rolling-std K=100 overlay on a twin y-axis, not thumbnails) — see `output/prime/20260818_021335/layer3_regime_wave_gallery_detailed.png`. Still purely qualitative, no new statistical claim.
...
## Five-Stage Session: pi Coefficient, Log-Detrend Residual, Phase/Changepoint, Hilbert, Regime-0 Fit

**Date:** August 23, 2026

**Framing:** this session ran five independent checks against the 20,000-prime gap sequence, one of which (Log-Detrend Residual) is the objective CLAUDE.md's "Next Session" note recorded back on 2026-08-18 and left unstarted through the following session's exploratory detour. None of the five retest cross-regime self-similarity (still refuted, see Regime Overlay above) — they ask new questions about the same rolling-mean/std trend and the 39-changepoint set from `output/prime/20260818_015045/results.json`.

**1. pi/e Coefficient Test (priority stage).** `layer3_pi_coefficient_test.py`, `output/prime/20260823_125001/`. Fit `std(N) ~ a*ln(N+2)+b` and `variance(N) ~ a*ln(N+2)+b` (K=100 rolling window, full 20k sequence) and compared the free-fit slopes against five candidate constants (1/2pi, 1/pi, pi^2/6 for variance, 1/2e, ln(2)/pi). **Result: REFUTED, cleanly.** Empirical std slope a=1.1026 (R^2=0.567), variance slope a=17.589 (R^2=0.490) — every candidate misses by 71–91% relative error, none within the 1% flag threshold. No pi/e-based constant explains this growth rate; it's an ordinary fitted log-linear coefficient, consistent with PNT-driven growth already established elsewhere in this file, not a hidden mathematical constant. **Naming note:** the session prompt that requested this labeled the std fit "H(N)" — that collides with the unrelated Shannon-entropy H(N) in `experiments/gap_entropy_windows.py`; this script's std(N) is a different quantity, flagged explicitly to avoid the two being conflated later.

**2. Log-Detrend Residual Analysis** (the queued objective). `layer3_log_detrend_residual.py`, latest run `output/prime/20260823_130015/`. Fit log curves to the full-sequence rolling mean (`a=1.1047, R^2=0.778`) and rolling std (`a=1.1026, R^2=0.567`), subtract to get two residual series, then run FFT, autocorrelation, and sliding-window kurtosis on each.

**Methodological catch worth recording on its own:** the first version of this test used a full i.i.d.-shuffle permutation null and returned "moderate confidence, significant structure" on both residuals at p≈0. That was wrong — the rolling-mean/std series is built from a step=1, K=100 *overlapping* window, so adjacent values share 99/100 of their underlying data by construction, producing near-1 autocorrelation at low lag with zero dependence on any real signal (a triangular, boxcar-induced artifact, decaying to ~0 only past lag K). A full shuffle destroys that trivial construction artifact along with everything else, so it has no discriminating power — same failure class as the two Layer 2 zero-crossing proximity checks in `hypotheses/second_order_gap_structure.md`. Fixed with a **block-shuffle null** (non-overlapping blocks of 200, block order permuted, block contents intact) that preserves the trivial windowing correlation in every null draw so it stops driving significance.

**Result after the fix: no structure survives detrending.** FFT top-peak permutation test: not significant for either residual (mean-residual p=0.237, std-residual p=0.156). Autocorrelation: nominally "significant" at p<0.0001 for both, but only because the block-shuffle null's variance is extremely tight (std≈0.0004) — the actual gap between observed and null mean is a 0.40–0.46% relative effect size, well under the 2% threshold set for calling it non-negligible. **Confidence, stated plainly: none for either residual** — statistically detectable but practically meaningless, the same "real but tiny" pattern already seen in this session's-predecessor's Global Damped-Oscillation check (CLAUDE.md session handoff, 2026-08-18/19: R^2=0.0103, p=0.006). This closes out the CLAUDE.md-queued question: once the PNT-driven log trend is removed from both the rolling mean and rolling std, nothing else is left in either series.

**3. Phase Analysis at 39 Changepoints.** `layer3_phase_changepoint_analysis.py`, `output/prime/20260823_130641/` (superseding an earlier same-day run at `20260823_130243` that reported STFT phase-jump p-values for components the method couldn't actually resolve — see catch below). Global FFT of the full sequence, both raw (mean-subtracted only) and detrended (own best-fit line removed) — the raw variant's top 3 components land at bins 1–3 (periods ≈20000, ≈10000, ≈6666), the same FFT trend-leakage artifact already diagnosed as the "regime 0 FFT artifact" elsewhere in this file; not real periodicity. The detrended variant surfaces two short, plausibly-real periods (2.29 and 7.56 gap-steps, consistent with the known short-range parity/alternation structure already documented in the Per-Regime Characterization follow-up above) alongside one residual long-period component (still period≈20000 — a straight line can't fully remove a logarithmic trend).

**Second methodological catch, caught from the plot itself before trusting the numbers:** the STFT local-phase test (width=500 window) was originally run on all 6 components (3 per variant) uncritically. The three period≈6666–20000 components' phase panels showed an erratic square-wave bouncing between 0 and pi rather than a smooth trajectory — a window of 500 samples cannot contain even one cycle of a ~10,000–20,000-period oscillation, so the "nearest frequency bin" lookup degenerates to the window's own near-DC bin, and the resulting "phase" just reflects the sign of local noise, not a real instantaneous phase. Added an explicit resolvability check (`period <= width/2`) and now label those results NOT INTERPRETABLE rather than let a meaningless p-value stand as if it were evidence.

**Result, for the two components the test can actually resolve (both short-period, detrended):** period 2.29, p=0.056; period 7.56, p=0.590 — **neither significant.** No detectable phase discontinuity at the 39 changepoints for either resolvable frequency. The four unresolvable components (all period ≥6666) simply can't be tested this way at this scale; this is a limitation of the method, not a null result for those frequencies.

**4. Hilbert Transform / Analytic Signal.** `layer3_hilbert_analytic_signal.py`, `output/prime/20260823_130412/`. Applied `scipy.signal.hilbert` directly to the raw (untouched) gap sequence, per the literal instruction, despite the sequence being strictly positive and trend-dominated rather than the zero-mean/narrowband signal the transform is usually applied to — flagged as a confound before running, not after.

- **Changepoint phase-jump test** (same permutation design as Stage 3, applied directly to the Hilbert instantaneous phase rather than an STFT-derived one): p=0.629, **not significant.**
- **Envelope vs. Stage-2 log-trend fit:** pre-registered expectation was a high correlation (a trend-dominated positive signal's envelope should track its own magnitude). Actual result: **r=0.168, weak — contrary to the stated expectation**, reported as a genuine (mild) surprise rather than adjusted after the fact. Also noted: the "vs. mean fit" and "vs. std fit" correlations are mathematically identical, not two independent checks — both curves are positive-slope affine transforms of the same `ln(N+2)`, and Pearson r is invariant to that.

**5. Regime-0 Functional Form Fit.** `layer3_regime0_functional_fit.py`, `output/prime/20260823_130456/`. Fit log/power/linear/exponential (all 2-parameter, via `curve_fit`) to raw gaps in regime 0 under both boundary definitions that already exist in this repo: `[0,499)` (the 40-regime carving's first segment) and `[0,1529)` (the original 3-regime carving's first segment). **Log wins by both AIC and BIC under both boundary definitions** — `[0,499)`: AIC 1591.39 vs. 1591.88/1597.31/1598.31 for power/linear/exponential; `[0,1529)`: AIC 5481.64 vs. 5482.54/5494.77/5496.52. **The boundary definition does not change the winner.** All R² are low (0.023–0.047, expected for a fit against noisy raw per-gap values rather than a smoothed series), but the relative ranking across the four candidate forms is stable and consistent with this file's own "Layer 1 — Envelope: Logarithmic function governs regime spacing" framing.

**Net read across all five stages:** the log-growth trend (mean and std both ~ln(N)) remains the only real, non-negligible structure found in the full 20k sequence — it isn't explained by any pi/e constant, nothing survives once it's subtracted out, phase at the changepoints shows no discontinuity by either of two independent tests, and it's still the best-fitting functional form for regime 0's own shape regardless of which of the two established boundary definitions is used. No result in this session reopens the refuted cross-regime self-similarity finding or any of the other null results already on record in this file.

**Caveats:**
1. Both methodological catches above (the block-shuffle null in Stage 2, the STFT resolvability check in Stage 3) were caught and fixed within this session before being committed as final — recorded here specifically so a future session doesn't have to rediscover either pitfall.
2. The Stage 3/4 phase-discontinuity tests only check for a *local jump* in phase near each changepoint; they don't test for a phase *offset* or slow drift, which would need a different test design.
3. Stage 5's AIC/BIC comparison is between four specific 2-parameter forms — a richer form (e.g. `a*ln(N)+c*N+b`) was not tried and could fit better than any of the four by construction (more parameters), which is exactly why AIC/BIC (which penalize parameter count) rather than raw R² was used for the comparison in the first place.
