...
- --shots not given: queue depth 1 <= 5 threshold, using 4096 shots
- WARNING: transpiled depth 583 exceeds the 50-gate threshold -- deep circuits accumulate noise fast, expect the hardware distribution to diverge more from the simulated one.
...
- WARNING: transpiled depth 907 exceeds the 50-gate threshold -- deep circuits accumulate noise fast, expect the hardware distribution to diverge more from the simulated one.
- Readout error mitigation: measurement twirling enabled (no added circuit depth).
