# Entropy velocity and residual-amplitude decay -- 20260818_235347
...
**Statistically significant but practically negligible.** The inv_sqrt model's improvement over the flat baseline is unlikely to arise by chance (permutation p=0.0060, well below 0.05 -- this bar was cleared), but the effect size is tiny (R^2=0.0103, below the 0.1 threshold used here to call an effect size meaningful). With n=757 rolling-std points, even a very small real effect can be statistically distinguishable from a shuffled null -- that is what's happening here, not evidence of a practically meaningful decay. Read as: the inv_sqrt shape is very slightly, but genuinely, better than flat -- not as: residual amplitude meaningfully decays with N.
